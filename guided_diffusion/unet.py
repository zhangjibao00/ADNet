...
class Encoder_S(nn.Module):
    def __init__(self):
        ...
        kwargs_gate0 = {'kernel_size': 1, 'stride': 1, 'padding': 0}
        self.nonlinearity = nn.LeakyReLU(0.1)
        self.pool = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)
        self.pool0 = nn.AvgPool2d(kernel_size=1, stride=1, padding=0)
        self.gate_nonlinearity = nn.Sigmoid()

        # encoder0
        self.conv1 = (nn.Conv2d(6, 32, **kwargs))
        self.conv2 = (nn.Conv2d(32, 32, **kwargs))
        self.bypass1 = (nn.Conv2d(6, 32, **kwargs_short))
        self.model1 = nn.Sequential(self.conv1, self.nonlinearity, self.conv2, self.pool0)
        self.shortcut1 = nn.Sequential(self.pool0, self.bypass1)
        self.gateconv1 = (nn.Conv2d(6, 32, **kwargs_gate0))
        self.gate1 = nn.Sequential(self.gateconv1, self.gate_nonlinearity)

        # encoder1
        self.conv3 = (nn.Conv2d(32, 32, **kwargs))
        self.conv4 = (nn.Conv2d(32, 64, **kwargs))
        self.shortcut2 = (nn.Conv2d(32, 64, **kwargs_short))
        self.model2 = nn.Sequential(self.nonlinearity, self.conv3, self.nonlinearity, self.conv4)
        self.gateconv2 = (nn.Conv2d(32, 64, **kwargs_gate))
        self.gate2 = nn.Sequential(self.gateconv2, self.gate_nonlinearity)

        # encoder2
        self.conv5 = (nn.Conv2d(64, 64, **kwargs))
        self.conv6 = (nn.Conv2d(64, 128, **kwargs))
        self.shortcut3 = (nn.Conv2d(64, 128, **kwargs_short))
        self.model3 = nn.Sequential(self.nonlinearity, self.conv5, self.nonlinearity, self.conv6)
        self.gateconv3 = (nn.Conv2d(64, 128, **kwargs_gate0))
        self.gate3 = nn.Sequential(self.gateconv3, self.gate_nonlinearity)

        # encoder3
        self.conv7 = (nn.Conv2d(128, 128, **kwargs))
        self.conv8 = (nn.Conv2d(128, 256, **kwargs))
        self.shortcut4 = (nn.Conv2d(128, 256, **kwargs_short))
        self.model4 = nn.Sequential(self.nonlinearity, self.conv7, self.nonlinearity, self.conv8)
        self.gateconv4 = (nn.Conv2d(128, 256, **kwargs_gate))
        self.gate4 = nn.Sequential(self.gateconv4, self.gate_nonlinearity)

        # encoder4
        self.conv9 = (nn.Conv2d(256, 256, **kwargs))
        self.conv10 = (nn.Conv2d(256, 512, **kwargs))
        self.shortcut5 = (nn.Conv2d(256, 512, **kwargs_short))
        self.model5 = nn.Sequential(self.nonlinearity, self.conv9, self.nonlinearity, self.conv10)
        self.gateconv5 = (nn.Conv2d(256, 512, **kwargs_gate0))
        self.gate5 = nn.Sequential(self.gateconv5, self.gate_nonlinearity)

        # encoder5
        self.prior_conv1 = (nn.Conv2d(512, 512, **kwargs))
        self.prior_conv2 = (nn.Conv2d(512, 512, **kwargs))
        self.prior_shortcut = (nn.Conv2d(512, 512, **kwargs_short))
        self.prior_model = nn.Sequential(self.nonlinearity, self.prior_conv1, self.nonlinearity, self.prior_conv2)
        self.gateconv6 = (nn.Conv2d(512, 512, **kwargs_gate))
    ...
    def forward(self, x):
        ...
        feature.append(x)
        x = self.nonlinearity(self.pool0(self.model3(x)) + self.pool0(self.shortcut3(x))) * self.gate3(x)
        feature.append(x)
        x = self.nonlinearity(self.pool(self.model4(x)) + self.pool(self.shortcut4(x))) * self.gate4(x)
        feature.append(x)
        x = self.nonlinearity(self.pool0(self.model5(x)) + self.pool0(self.shortcut5(x))) * self.gate5(x)
        feature.append(x)
...
class CustomModel(nn.Module):
    def __init__(self, tongdao=128, output_channels=128, output_size=(64, 64)):
        super(CustomModel, self).__init__()

        self.output_size = output_size  # 目标输出尺寸

        # 定义 1x1 卷积层，输出通道数根据output_channels参数控制
        self.conv1x1_1 = nn.Conv2d(in_channels=tongdao * 2, out_channels=output_channels, kernel_size=1)
        self.conv1x1_2 = nn.Conv2d(in_channels=tongdao * 2, out_channels=output_channels, kernel_size=1)

        # 定义膨胀卷积
        self.dilated_conv_1 = nn.Conv2d(output_channels, output_channels, kernel_size=3, dilation=1, padding=1)
        self.dilated_conv_2 = nn.Conv2d(output_channels, output_channels, kernel_size=3, dilation=2, padding=2)
        self.dilated_conv_3 = nn.Conv2d(output_channels, output_channels, kernel_size=3, dilation=3, padding=3)

        # Softmax操作
        self.softmax = nn.Softmax(dim=1)

        # 定义Slice操作（通道拆分）
        self.slice = nn.ModuleList([nn.Conv2d(output_channels, 1, kernel_size=1) for _ in range(4)])

        # 定义3x3卷积
        self.conv3x3_1 = nn.Conv2d(1, output_channels, kernel_size=3, padding=1)
        self.conv3x3_2 = nn.Conv2d(1, 1, kernel_size=3, padding=1)
        ...
        # 假设输入的特征尺寸为64x64，计算中间层的步幅
        self.upsample = nn.Upsample(size=(self.target_height, self.target_width), mode='bilinear', align_corners=False)

    def forward(self, x1, x2):
        # 将输入在通道上拼接
        x = torch.cat([x1, x2], dim=1)  # 拼接后的通道数为x1和x2的通道数之和
        ...
        # 膨胀卷积操作
        dilated_1 = self.dilated_conv_1(out1)
        dilated_2 = self.dilated_conv_2(out1)
        dilated_3 = self.dilated_conv_3(out1)

        # Softmax操作
        softmax_out = self.softmax(out2)

        # 通道拆分（slice操作）
        W1 = self.slice[0](softmax_out)
        W2 = self.slice[1](softmax_out)
        W3 = self.slice[2](softmax_out)
        W4 = self.slice[3](softmax_out)

        # W1, W2, W3与膨胀卷积结果相乘并相加
        weighted_sum = W1 * dilated_1 + W2 * dilated_2 + W3 * dilated_3

        # 归一化
        norm_weighted_sum = F.normalize(weighted_sum, p=2, dim=1)
        ...
        # a与归一化结果相乘，b加到结果
        final_result = a * norm_weighted_sum + b

        # 最后一个1x1卷积和跳跃连接
        final_result = self.final_conv(final_result)
        output = final_result + out1 + out2  # 跳跃连接
...
class UpsampleDeconvNet(nn.Module):
    def __init__(self):
        super(UpsampleDeconvNet, self).__init__()

        self.deconv1 = nn.ConvTranspose2d(in_channels=512, out_channels=256, kernel_size=4, stride=2, padding=1)
        self.deconv2 = nn.ConvTranspose2d(in_channels=512, out_channels=128, kernel_size=3, stride=1, padding=1)
        self.deconv3 = nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.deconv4 = nn.ConvTranspose2d(in_channels=128, out_channels=32, kernel_size=4, stride=2, padding=1)
        self.deconv5 = nn.ConvTranspose2d(in_channels=32, out_channels=16, kernel_size=4, stride=2, padding=1)
        self.deconv6 = nn.ConvTranspose2d(in_channels=32, out_channels=3, kernel_size=3, stride=1, padding=1)

    def forward(self, x, output1, output2, output3):
        x = self.deconv1(x)
...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        # 添加
        self.encoderd = Encoder_T()
        self.encoderm = Encoder_S()
        # self.net_D = GlobalDiscriminator()
        self.model_1 = CustomModel(tongdao=512, output_channels=256, output_size=(64, 64))
        self.model_2 = CustomModel(tongdao=512, output_channels=64, output_size=(64, 64))
        self.model_3 = CustomModel(tongdao=512, output_channels=16, output_size=(256, 256))
        self.model0 = UpsampleDeconvNet()
        self.cross_attenstion = Cross_Attention()
        self.cross_attenstion0 = Cross_Attention()
        self.cross_attenstion1 = Cross_Attention()
        #
    ...
    def forward(self, x, timesteps, y=None):
        ...
        mask = h[:, 9:12, :, :]
        c = th.cat((img_m, img_s), dim=1)
        c1 = th.cat((img_m, mask), dim=1)
        c2 = th.cat((mask, img_s), dim=1)
        h = th.cat((img_truth, img_m), dim=1)
        anch, cal = self.highway_forward(c)

        # 添加
        s_feature = self.encoderd(c1)
        t_feature = self.encoderm(c2)
        ...
        distribution = self.cross_attenstion(h, t_feature[-1])
        distribution0 = self.cross_attenstion0(h, s_feature[-1])

        output1 = self.model_1(distribution, distribution0)
        output2 = self.model_2(distribution, distribution0)
        output3 = self.model_3(distribution, distribution0)
        ...
        distribution1 = self.cross_attenstion1(distribution, distribution0)
        ...
        h = h+distribution1
        out = self.model0(h, output1, output2, output3)
        # for module in self.output_blocks:
        #     h = th.cat([h, hs.pop()], dim=1)
        #     h = module(h, emb)
        # h = h.type(x.dtype)
