...
class CreateDataset(data.Dataset):
    def __init__(self, opt):
        ...
        self.structure_paths, self.structure_size = make_dataset(opt.structure_file)
    ...
    def __getitem__(self, index):
        ...
        # load structure
        structure = self.load_structure(index)
        return {'img': img, 'img_path': img_path, 'mask': mask, 'structure': structure}
    ...
    def load_structure(self, index):
        ImageFile.LOAD_TRUNCATED_IMAGES = True
        img_path = self.structure_paths[index % self.structure_size]
        img_pil = Image.open(img_path).convert('RGB')
        img = self.transform(img_pil)
        img_pil.close()
        return img
...
def dataloader(opt):
    ...
    dataset = data.DataLoader(datasets, batch_size=opt.batch_size, shuffle=not opt.no_shuffle, num_workers=int(opt.nThreads))
